---
title: "Economic Security GLOA 610"
...
# .huge[**.white[Economic Security] <br> .tiny[&] <br>.pink[De].yellow[globalization]**]
...
## - or something else? (#.pink[reshoring]): also relevant to .yellow[economic security]

---

# .red[de]globalization & economic security
...
.blockquote[## **.green[Economic Security:]**
.black[.Large[
  
- the capacity to reliably access, produce, and distribute essential goods and services 
  
- .Blue[despite] external shocks, such as geopolitical conflict, pandemics, or trade restrictions. 
...
background-image: url(https://images.axios.com/-qa9N1UTk3rfiXjmHVHonoKs3EM=/2023/12/07/1701967998497.jpg)
background-size: 110%


# .Huge[
.yellow[**Deglobalization exposes <br>economic .blue[in]security.**]]
# ('choke point'; 'weaponized interdependence')
...
# .Large[Are we on a .hl1[long-term] downward trend <br> (in terms of globalization)?]
...
---
# So really, what sanctions using .blue[SWIFT] does:

.pull-left[
.Large[.red[Hey, move my $ out of that bank!]]
```{r, out.width="75%"}
knitr::include_graphics("https://thumbs.dreamstime.com/z/cartoon-walking-man-businessman-making-phone-call-old-table-phone-cartoon-stick-drawing-conceptual-illustration-121201785.jpg")
```
]
...
.pull-right[
```{r, out.width="90%"}
knitr::include_graphics("https://d7hftxdivxxvm.cloudfront.net/?height=801&quality=1&resize_to=fit&src=https%3A%2F%2Fd32dm0rphc51dk.cloudfront.net%2FjZnRjedFIVB2-H5qth6iSA%2Fsmall.jpg&width=801")
```
]
...
> .Large[A notable jump in CIPS usage in Q2, per lates PBOC data release. The system processed 1.62 million transactions worth 29.49 trillion RMB (~$4 trillion). A 60% and 29% increase, respectively, year-over-year.] (@daniel_mcdowell)
...
- .LARGE[Dollar Dominance persists.]

  - .Large[liquidity]
  - .Large[capital openness]
  - .Large[credibility]

--

- .LARGE[Some considerations:]
...
# Create data (with columns already switched)
df <- tibble::tibble(
  Category = c("Russia; China", "Fintech platforms", "CBDCs"),
  Challenge = c("No Challenge", "No challenge", "Maybe?"),
  Notes = c("Limited coverage; capital control",
            "Domestic; isolated",
            "High potential (early stage)")
)

df %>%
  kbl(align = "l", booktabs = T) %>%
  kable_styling(full_width = F,
                font_size = 27) %>%
  column_spec(1, bold = F, width = '12em') %>%
  column_spec(2, background = '#ED5132', 
              color='white', width = '10em') %>%
  column_spec(3, bold = F, width = '25em')
...
background-image: url(https://github.com/textvulture/textvulture.github.io/blob/master/images/AppleFT.png?raw=true)
background-size: 90%
...
- .LARGE['Containment' policies are designed .green[hastily] and with .purple[hope] of global coordination.]

--

  - .Large[China: supercomputing + AI = hypersonic, stealth, and decryption weapons] (NYT 2022)

--

- .LARGE[US export control exemptions: short- `r fa("arrow-right")` .red[long]-term] (WSJ 2022)

--

  - .Large[chip industries highly dependent with each other.]

--

- .LARGE[Some firms and countries push back]

--

  - .Large[ASML, TSMC, Samsung]
...
class: middle
background-image: url(https://www.imf.org/-/media/images/imf/fandd/charts/2023/agarwal-chart1-updated.png?w=1136)
background-size: contain
background-position: right
...
## Things that .red[cannot] be reshored? (e.g., .blue[cobalt]) (1)
...
## Things that .red[cannot] be reshored? (e.g., .blue[cobalt]) (2)
