---
title: "Crisis, GLOA 610"
author: "Byunghwan Son <br> George Mason University"
date: " "
#########################################
linkcolor: red
citecolor: blue
urlcolor: blueß
output: 
  xaringan::moon_reader:
    seal: false
    nature:
      ratio: '16:9'
      highlightStyle: github
  #    self-contained: TRUE
  chakra: css/libs/remark-latest.min.js
css: ["default", 
      "css/footer.css", 
      "css/color.css", 
      # "css/xaringan-themer.css",
      default-fonts]
lib_dir: css/libs
beforeInit: "css/macros.js"
# seal: false  
#bibliography: /Users/bson3/Dropbox/latex/mybib2.bib
#remotes::install_github('yihui/xaringan', upgrade = TRUE)
---

```{r setup, include=FALSE}
# to use FontAwesome
#htmltools::tagList(rmarkdown::html_dependency_font_awesome())
library(fs)
library(fontawesome)
# library(magick)  
library(ggplot2)
library(tidyverse)
```

class: inverse, center, middle
background-image: url(https://www.timeshighereducation.com/sites/default/files/styles/the_breaking_news_image_style/public/istock-530583132.jpg?itok=CekMzkG3)
background-size: cover

# .HUGE[**Global Economic Crisis**] <br><br> **.green[GLOA] .yellow[610]**

---
class: inverse
background-image: url(https://www.brunswickgroup.com/media/3062/crisis.jpg?anchor=center&mode=crop&heightratio=0.5625&width=767&rnd=131871899560000000)
background-size: cover


# .Large[What is a .red[CRISIS] everybody talks about?]

--

## `r fa("microphone-alt")` &nbsp; Marxist Perspective (deterministic)

--

## `r fa("microphone-alt")` &nbsp; Critical Theory Perspective (e.g., .yellow[D&#246;nmez & Zemandl 2019])

--

## `r fa("microphone-alt")` &nbsp; Neoclassical Economics Perspective ('*what crisis?*')

--

## - .red[Not wrong], but offer only a partial picture; little to no solution 

# .yellow[`r fa("exclamation-circle")`] &nbsp; A .orange[big-picture] framework is rare.

---
class: inverse, middle, center
background-image: url(https://epe.brightspotcdn.com/dims4/default/cfee7fb/2147483647/strip/true/crop/589x400+5+0/resize/840x570!/quality/90/?url=https%3A%2F%2Fepe-brightspot.s3.amazonaws.com%2F20%2F8d%2Fbc180f541b3e7d17d138e4c3b9c5%2F37modechay-img.jpg)
background-size: cover

# .HUGE[**Types of Crisis**]

---

class: inverse, middle, center
background-image: url(https://wickedgoodgaming.com/wp-content/uploads/2019/10/bankrupt.gif)
background-size: cover

# .huge[**(Sovereign) Debt Crisis**]

# *'The government cannot service the debt anymore'*

---

class: inverse, middle, center
background-image: url(https://image.shutterstock.com/image-illustration/paris-police-hold-back-crowd-260nw-242294071.jpg)
background-size: cover

# .huge[**Banking Crisis**]

# *'The bank doesn't have your money.'*

---

class: inverse, middle, center
background-image: url(http://prod-upp-image-read.ft.com/8a4b2c44-c350-11e5-808f-8231cd71622e)
background-size: cover

# .huge[**Currency Crisis**]

# *'The currency is now just a piece of paper.'*

---


```{r include=FALSE, cache=T}

library(WDI)
library(lubridate)

var = c('crisis' = "GFDD.OI.19",
        'GDPpc' = 'NY.GDP.PCAP.KD',
        'exrate' = 'PA.NUS.FCRF')


wdi <- WDI(indicator=var,
           start=1960,
           country = 'all',
           end=2020,
           extra=TRUE)

```

<div class="full-width-tinted"><br>
  <h1> .large[Banking Crisis Trend, World Bank Data ] </h1> 
</div><br><br> 

```{r message=FALSE, echo=FALSE, warning=FALSE, results="asis", fig.width=16, fig.height=7, cache=T}

wdi %>%
  filter(GDPpc < 10000) %>%
  drop_na(crisis) %>%
  group_by(year) %>%
  summarise(y.crisis = sum(crisis)) -> wdi2

wdi %>%
  drop_na(crisis) %>%
  group_by(year) %>%
  summarise(y.crisis = sum(crisis)) %>%
  ggplot(aes(y=y.crisis, 
             x=year)) +
  geom_bar(aes(fill='developed'),
           stat = "identity",   alpha=0.5) +
  geom_bar(data=wdi2, 
           aes(x=year, y=y.crisis, fill='underdeveloped'), 
           stat = "identity") +
    labs(y="# of Banking Crises") +
  theme_minimal() +
  theme(legend.title = element_text(size=0),
        legend.text = element_text(size=25),
        axis.text=element_text(size=25),
        axis.title=element_text(size=25))

```

---

<div class="full-width-tinted"><br>
  <h1> .large[Currency Crisis Trend, more than 10% depreciation] </h1> 
</div><br><br> 

```{r message=FALSE, echo=FALSE, warning=FALSE, results="asis", fig.width=16, fig.height=7, cache=T}

library(plm)


wdi <- pdata.frame(wdi, index=c("country", "year"))


wdi %>%
  transform(lexrate = plm::lag(wdi$exrate)) %>%
  mutate(dep = (exrate-lexrate)/exrate) %>%
  mutate(ccrisis = 
           case_when(dep > 0.1 ~ 1,
                     dep < 0.0999999 ~ 0,
                     is.na(dep) ~ 0
           )) %>%
    filter(GDPpc<10000) %>%
  drop_na(ccrisis) %>%
  group_by(year) %>%
  summarise(y.ccrisis = sum(ccrisis)) -> wdi2

wdi %>%
  transform(lexrate = plm::lag(wdi$exrate)) %>%
  mutate(dep = (exrate-lexrate)/exrate) %>%
  mutate(ccrisis = 
           case_when(dep > 0.1 ~ 1,
                     dep < 0.0999999 ~ 0,
                     is.na(dep) ~ 0
           )) %>%
  drop_na(ccrisis) %>%
  group_by(year) %>%
  summarise(y.ccrisis = sum(ccrisis)) %>%
  ggplot(aes(y=y.ccrisis, 
             x=year)) +
  geom_bar(aes(fill='developed'),
           stat = "identity",   alpha=0.5) +
  geom_bar(data=wdi2, 
           aes(x=year, y=y.ccrisis, fill='underdeveloped'), 
           stat = "identity") +
  labs(y="# of Currency Crises") +
  theme_minimal() +
  theme(legend.title = element_text(size=0),
        legend.text = element_text(size=15),
        axis.text=element_text(size=15),
        axis.text.x = element_text(angle = 75,
                                   vjust = 0.5),
        axis.title=element_text(size=15)) 



```

---

class: inverse
background-color: black

<div class="full-width-tinted"><br>
  <h1> .large[Just .red[how] does a financial crisis happen?] </h1> 
</div><br><br> 

.right-column[

```{r echo=F, out.height=450, out.width=700}
knitr::include_graphics("https://stray-reflections.com/storage/images/articles/16068579750.png")
```

]

.left-column[

### Kindleberger (1979)

### - .yellow[mania]

### - .orange[panic]

### - .red[crash]

.black[]
]

---

class: inverse
background-color: black


<div class="full-width-tinted"><br>
  <h1> .large[Just .red[how] does a financial crisis happen?] </h1> 
</div><br><br> 


.right-column[
.center[
```{r echo=F, out.height=450, out.width=700}
knitr::include_graphics("https://z822j1x8tde3wuovlgo7ue15-wpengine.netdna-ssl.com/wp-content/uploads/2013/11/131102-08.png")
```
]
]

.left-column[
### Kindleberger (1979)

### - .yellow[mania]

### - .orange[panic] 

### - .red[crash]

.black[]
]


---

class: inverse
background-color: black


<div class="full-width-tinted"><br>
  <h1> .large[Just .red[how] does a financial crisis happen?] </h1> 
</div><br><br> 


.right-column[
.center[
```{r echo=F, out.height=450, out.width=700}
knitr::include_graphics("https://z822j1x8tde3wuovlgo7ue15-wpengine.netdna-ssl.com/wp-content/uploads/2013/11/131102-08.png")
```
]
]

.left-column[
### Kindleberger (1979)

### - .yellow[mania]

### - .orange[panic] 

### - .red[crash]

### Oatley (2019)

- .large[.red[complex] system approach]

.black[]
]


---

class: inverse
background-color: black
background-image: url(https://c.tenor.com/5Egq59EfO1MAAAAC/dont-cry-for-me-argentina-evita.gif)
background-position: right
background-size: 77%

# But, wouldn't governments <br> .yellow[rein in]  when the bubble <br> builds up?

--

## or <br><br>  are there <br> some .red[easier] options?

---

## 1) Cook up the numbers: Aragao and Linsi (2022, 99)
.center[
![](https://github.com/textvulture/textvulture.github.io/blob/master/images/Aragao2020.png?raw=true)
]

---
class: inverse, middle

background-image: url(https://www.economist.com/img/b/300/291/90/sites/default/files/images/print-edition/20120225_AMC632.gif)
background-position: right
background-size: contain
background-color: #C6D9DF

# .huge[**... which <br> became <br>an Endemic <br> Problem**]

---

class: inverse, middle
background-image: url(https://i.guim.co.uk/img/media/a4ca6635c18aecb768584d5d90cfc3b6fbb38540/0_96_6000_3600/master/6000.jpg?width=465&quality=45&auto=format&fit=max&dpr=2&s=b499693ac35275058616a168cc3cd256)
background-size: cover
background-color: black

# .huge[**2) and blame the IMF*.**] <br> .right[(Doesn't mean IMF is innocent...)]


---

class: inverse
background-color: black

<div class="full-width-tinted"><br>
  <h1> Consequences and Implications </h1> 
</div><br><br> 

--

## `r fa("lightbulb", fill='yellow')` Explains why .yellow[state]-oriented policies are rare: &nbsp; `r fa("bomb")` boom-and-bust <br>

--

![](https://github.com/textvulture/textvulture.github.io/blob/master/images/Campello2014.png?raw=true)
